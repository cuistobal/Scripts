...
sorted_functions_file="sorted_functions"
...
# Sort the functions file and save it to sorted_functions_file
awk '
{
    # Extract the first alphanumeric word (function name)
    match($0, /^[a-zA-Z0-9_]+/, matches);
    if (matches[0]) {
        print matches[0] "|" $0;  # Store the function name as a key for sorting
    }
}' "$functions_file" | sort -t "|" -k1,1 | cut -d "|" -f2 > "$sorted_functions_file"
...
echo "Sorted functions have been saved to '$sorted_functions_file'."
